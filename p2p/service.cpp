...
bool Service::connect_peer(const std::string& ip, std::uint16_t port) {
    ...
    if(is_connected_session(ip, port)) {
        log_info(cortono::util::format("(%s:%u) has connected, ignore...", ip.data(), port));
        return false;
    }
    if(is_local_peer(ip, port)) {
        log_info(cortono::util::format("(%s:%u) is local peer, ignore...", ip.data(), port));
        return false;
    }

    peer_manager_.append(ip, port);
    if(session_manager_.server_session_size() >= configuration_.max_connection_count) {
        log_info(cortono::util::format("connection count(%d) is enough", session_manager_.server_session_size()));
        return false;
    }
    ...
    auto client = cortono::net::TcpClient::connect(
        service_.acquire_eventloop(), ip, port, std::move(read_cb), nullptr, std::move(close_cb), std::move(conn_cb));
...
void Service::handle_close(const cortono::net::TcpConnection::Pointer& conn) {
    ...
    if(!session_manager_.exist(ip, port)) {
        return;
    }
    log_info("close connection", conn->name());
    std::size_t server_session_count = 0;
    bool with_server{ false };
    {
        with_server = session_manager_.get(ip, port)->with_server();
        server_session_count = session_manager_.server_session_size();
        session_manager_.remove(ip, port);
        peer_manager_.remove(ip, port);
    }
    if(server_session_count >= configuration_.max_connection_count && with_server) {
        maintain_connection_count();
        // boardcast_to_network(GetAddrDatagram{});
    }
    print_connected_address();
}
...
void Service::print_connected_address()  {
